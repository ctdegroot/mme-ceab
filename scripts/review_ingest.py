...
def check_instructor(grid, add):
    ...
    for r in range(1, len(grid)):
        ...
        miss = [label for f, label in
                [("instructorID", "the login/username ID"), ("firstName", "first name"), ("lastName", "last name")]
                if is_blank(get(f))]
        if miss:
            add(ERROR, "missing_instructor",
                f"On the “1 - Instructor” tab, row {r + 1} is missing {join_list(miss)}",
                "Please fill in the instructor’s ID, first name, and last name.")
    if not any_row:
        add(ERROR, "missing_instructor",
            "The “1 - Instructor” tab has no instructor filled in",
            "Please enter the instructor’s ID, first name, and last name.")
...
def check_course(grid, add):
    ...
    for r in range(1, len(grid)):
        ...
        for f, label in [("courseID", "Course ID"), ("instructorID", "Instructor ID")]:
            if is_blank(get(f)):
                add(ERROR, "missing_course_field", f"{at}: {label} is blank", f"Please fill in the {label}.")
        prefix = get("prefix")
