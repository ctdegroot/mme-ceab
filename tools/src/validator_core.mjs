...
function checkInstructor(grid, add) {
  ...
  for (let r = 1; r < grid.length; r++) {
    ...
    any = true;
    const miss = [["instructorID", "the login/username ID"], ["firstName", "first name"], ["lastName", "last name"]]
...
function checkCourse(grid, add) {
  ...
  for (let r = 1; r < grid.length; r++) {
    ...
    for (const [f, label] of [["courseID", "Course ID"], ["instructorID", "Instructor ID"]]) {
      if (isBlank(get(row, f))) add(ERROR, "missing_course_field", `${at}: ${label} is blank`, `Please fill in the ${label}.`);
    }
